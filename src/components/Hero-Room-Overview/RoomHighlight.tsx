...
const rooms: RoomOverviewType[] = [
  ...
  },
  {
    ...
    description: "We love life at the beach. Being close to the ocean with access to endless sandy beach ensures a relaxed state of mind. It seems like time stands still watching ocean.",
          ...
          >
            <div className="lg h-[18rem] w-full border-none outline-none lg:flex lg:h-[35rem] lg:w-full lg:items-center lg:justify-center">
              <img
                ...
                className="shadow-soft h-full w-full rounded-[1rem] border-[0.2rem] border-[rgba(0,_0,_0,_0.2)] object-fill lg:h-[30rem] lg:w-[45rem] lg:rounded-[0.5rem] xl:h-[35rem] xl:w-[50rem]"
                loading="lazy"
              />
            </div>
            <div className="flex flex-col gap-[2rem] lg:flex lg:h-[full] lg:flex-col lg:justify-center">
              <div className="flex w-full flex-col items-center justify-center gap-[1rem] text-center text-secondary">
                <h1 className="font-secondary text-[1.8rem] font-bold lg:text-[2.5rem]">{room.header}</h1>
                <p className="w-[25rem] text-[1.1rem] font-[600] lg:w-[30rem] lg:text-[1.3rem]">{room.description}</p>
              </div>
              <div className="flex h-[5rem] w-full items-center justify-center">
                <button className="ease h-[4rem] w-[12rem] rounded-lg bg-primary text-[1.25rem] font-semibold uppercase tracking-wider text-white duration-200 hover:scale-110">Explore</button>
              </div>
            </div>
          </div>
        );
