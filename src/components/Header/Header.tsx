...
function Header() {
  ...
  const [headerPosition, setHeaderPosition] = useState<string>("absolute");
  const [burgerMenuVisibility, setBurgerMenuVisibility] = useState<boolean>(false);

  useEffect(() => {
    if (pathname.startsWith(paths.hero)) {
      setHeaderBackground("transparent");
      setHeaderPosition("absolute");
    } else if (pathname.startsWith(paths.rooms)) {
  ...
  return (
    <header
      className={`h-[10rem] w-full bg-${headerBackground} ${headerPosition} z-[4] flex justify-center`}
    >
